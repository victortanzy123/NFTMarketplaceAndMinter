...
contract ArtzoneMinterUpgradeable is
  ERC1155Upgradeable,
  ...
  UUPSUpgradeable
{
   ...
   * Initializer
   */
  function initialize(address[] memory _minters) external initializer {
    __ERC1155_init("");
    ...
    for (uint256 i = 0; i < _minters.length; ) {
      require(_minters[i] != address(0), "null address");
  ...
  function tokenMintedSupply(uint256 _tokenId) public view returns (uint256) {
    return _tokenSupply[_tokenId];
  }

  function tokenMaxSupply(uint256 _tokenId) public view returns (uint256) {
    return _tokenMaxSupply[_tokenId];
  }

  function tokenUpdateAccess(uint256 _tokenId) public view returns (bool) {
    ...
    returns (bool)
  {
    return
      ...
      ArtzoneMinter.supportsInterface(interfaceId) ||
